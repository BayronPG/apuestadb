...
- **Tablas del análisis de clase** (24/ago/2026): listado trabajado con el profesor (Empresa, Usuario→ROL, Login, Servicios, Deportes, Reglas, Liga, Apuestas, Equipo, Calendario, Hacer apuesta, Log de pago, Saldo o cuenta + atributos de Cliente). Borrador en `docs/base_datos/script_tablas_sqlserver_borrador.sql` (16 tablas). **Saldo por tipos tokens/PSE** (reglas R1-R5, indicación del profesor). No es modelo definitivo; falta validar Servicios, Reglas, Apuestas (oferta) y Resultado.
